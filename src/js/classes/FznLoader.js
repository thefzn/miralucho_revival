export default class FznLoader {
    ...
    go() {
        const width = this.fSize[0] * (1 - (this.game.loadQueue / this.total));
        const step = 5;
        this.size[0] += (width - this.size[0]) / step;
        [, this.size[1]] = this.fSize;
        this.redraw();
    }
    ...
    redraw() {
        ...
        if (this.source) {
            if (
                ...
                const ptrn = this.game.canvas.createPattern(
